...
def load_sym_points_idx(partnet_id, subdir):
    """Load symmetry points idx for a given PartNet object."""
    try:
        with open(f"{PSPSB_PATH}/{partnet_id}/{subdir}/sym_points_idx.json", "r") as f:
            symmetry_idx = json.load(f)
    except:
        data_paths = glob(f"{PSPSB_PATH}/{partnet_id}/{subdir}/symmetry_idx_*.pt")
        symmetry_idx = []
...
def load_sym_points_idx_gt(partnet_id, subdir):
...
def load_sym_icp_dist(partnet_id, subdir):
    ...
    try:
        with open(f"{PSPSB_PATH}/{partnet_id}/{subdir}/ICP_dist.json", "r") as f:
...
def compute_IoU(prediction, ground_truth, return_COV=False):
    """Compute Intersection over Union (IoU) and coverage (COV) for given predictions and ground truth."""
...
def save_data(partnet_id, subdir, icp_dist, IoU, cov):
    ...
    with open(f"{PSPSB_PATH}/{partnet_id}/{subdir}/metrics.json", "w") as f:
        json.dump(metrics, f)
...
for partnet_id in pbar:
    ...
    out_file = f"{PSPSB_PATH}/{partnet_id}/{subdir}/metrics.json"
    if os.path.exists(out_file):
        os.remove(out_file)
    #    continue  # skip model, if it was already processed

    # load precomputed symmetries
    groundtruth_idx = load_sym_points_idx(partnet_id, "ground_truth_approx")
    prediction_idx = load_sym_points_idx(partnet_id, subdir)

    if len(groundtruth_idx) == 0 or len(prediction_idx) == 0:
    ...
    IoU, cov = compute_IoU(prediction_idx, groundtruth_idx, return_COV=True)
