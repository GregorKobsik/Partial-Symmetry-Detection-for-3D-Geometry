...
from .utils import get_graph_feature, get_graph_feature_cross
...
class VNLeakyReLU(nn.Module):
    ...
    def forward(self, x):
        ...
        x_out = self.negative_slope * x + (1 - self.negative_slope) * (mask * x + (1 - mask) *
                                                                       (x - (dotprod / (d_norm_sq + EPS)) * d))
        return x_out
...
class VNLinearLeakyReLU(nn.Module):
    ...
    def forward(self, x):
        ...
        x_out = self.negative_slope * p + (1 - self.negative_slope) * (mask * p + (1 - mask) *
                                                                       (p - (dotprod / (d_norm_sq + EPS)) * d))
        return x_out
...
class VNLinearAndLeakyReLU(nn.Module):
    ...
    def __init__(self,
        ...
        self.leaky_relu = VNLeakyReLU(out_channels,
...
class VNStdFeature(nn.Module):
    ...
    def __init__(self, in_channels, dim=4, normalize_frame=False, share_nonlinearity=False, negative_slope=0.2):
        ...
        self.vn1 = VNLinearLeakyReLU(in_channels,
                                     in_channels // 2,
                                     dim=dim,
                                     share_nonlinearity=share_nonlinearity,
                                     negative_slope=negative_slope)
        self.vn2 = VNLinearLeakyReLU(in_channels // 2,
                                     in_channels // 4,
                                     dim=dim,
                                     share_nonlinearity=share_nonlinearity,
                                     negative_slope=negative_slope)
...
class VNEdgeConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, dim=5, n_knn=20, pooling='mean'):
        super().__init__()
        self.n_knn = n_knn

        self.conv = VNLinearLeakyReLU(in_channels, out_channels, dim)

        if pooling == 'max':
            self.pool = VNMaxPool(out_channels)
        elif pooling == 'mean':
            self.pool = VNMeanPool()
...
class VNEdgeConvCross(nn.Module):
